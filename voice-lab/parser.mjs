const HELPER_BINARY = "swift-helper/.build/debug/FlowStateHelper";
...
const ACTIVATION_VERBS = ["open", "launch", "focus", "switch", "activate", "start"];
...
function quoteJsonForShell(jsonObject) {
  return `'${JSON.stringify(jsonObject)}'`;
}

function buildHelperCommand(subcommand) {
  return `${HELPER_BINARY} ${subcommand}`;
}

function buildRunActionCommand(action) {
  return `${HELPER_BINARY} run-action ${quoteJsonForShell(action)}`;
}

function extractWindowId(rawTranscript) {
  const lower = rawTranscript.toLowerCase();

  const strictMatch = lower.match(/pid:\s*(\d+)\s*:\s*(index|window)\s*:\s*(\d+)/);
  if (strictMatch) {
    return `pid:${strictMatch[1]}:${strictMatch[2]}:${strictMatch[3]}`;
  }

  const spokenMatch = lower.match(/pid\s+(\d+)\s+(index|window)\s+(\d+)/);
  if (spokenMatch) {
    return `pid:${spokenMatch[1]}:${spokenMatch[2]}:${spokenMatch[3]}`;
  }
...
function extractFrameValues(normalizedTranscript) {
  ...
  const width = extractNumberWithRegex(normalizedTranscript, [
    /\bwidth\s*(\d+(?:\.\d+)?)/
  ]);
  const height = extractNumberWithRegex(normalizedTranscript, [
    /\bheight\s*(\d+(?:\.\d+)?)/
  ]);
...
function extractDirection(normalizedTranscript) {
  for (const direction of DIRECTION_WORDS) {
    if (normalizedTranscript.includes(direction)) {
      return direction;
    }
  }
...
function extractTarget(normalizedTranscript) {
  ...
  if (normalizedTranscript.includes("window")) {
    return "frontmost";
  }
...
function parseRaiseWindow(rawTranscript, normalizedTranscript) {
  const mentionsRaise = includesAny(normalizedTranscript, [
    "raise",
    "bring to front",
    "focus window"
  ]);
  ...
  const windowId = extractWindowId(rawTranscript);
  if (!windowId) {
    return failureResult(
      rawTranscript,
      normalizedTranscript,
      "Could not parse windowId for raise command.",
      "Include window id like: pid 37904 index 0"
    );
  }
...
function parseResizeWindow(rawTranscript, normalizedTranscript) {
  ...
  if (!windowId || values.width === null || values.height === null) {
    return failureResult(
      ...
      "Use: resize window pid 37904 index 0 width 900 height 700"
    );
  }
...
function parseSetFrame(rawTranscript, normalizedTranscript) {
  ...
  if (
    ...
    return failureResult(
      ...
      "Use: set frame window pid 37904 index 0 x 80 y 80 width 900 height 700"
    );
  }
...
function parseHelperMetaCommands(rawTranscript, normalizedTranscript) {
  if (includesAny(normalizedTranscript, ["request accessibility", "grant accessibility", "accessibility permission"])) {
    return successResult(
      ...
      buildHelperCommand("request-accessibility"),
      "Parsed accessibility request command."
    );
  }

  if (includesAny(normalizedTranscript, ["list windows", "show windows", "window list"])) {
    return successResult(
      rawTranscript,
      normalizedTranscript,
      "list_windows",
      buildHelperCommand("list-windows"),
  ...
  if (
    ...
    includesAny(normalizedTranscript, ["helper status", "flow state helper status", "flow helper status"])
  ) {
    return successResult(
      rawTranscript,
      normalizedTranscript,
      "helper_status",
      buildHelperCommand("status"),
...
export function parseVoiceTranscript(rawTranscript) {
  ...
  return failureResult(
    ...
    "Try: 'list windows' or 'set frame window pid 37904 index 0 x 80 y 80 width 900 height 700'"
  );
}
